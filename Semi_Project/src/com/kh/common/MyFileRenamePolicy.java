...
// 인터페이스 구현해야 함 -> implements 키워드 사용
public class MyFileRenamePolicy implements FileRenamePolicy{
	
	// 반드시 미완성된 rename이라는 추상메소드를 오버라이딩해서 구현해야 함!
	// 기존 파일을 전달받아서 파일명을 수정작업 후에 수정된 파일을 반환시켜줄 것
	// 매개변수 File 객체
	// 반환 File 객체
	...
	public File rename(File originFile) {
		...
		// 파일이 업로드된 시간(년월일시분초) + 5자리 랜덤값(100000 ~ 99999) + 확장자
		// 확장자 : 원본파일 그대로(원본파일명에서 뽑아서)
		
		/*
		 * 원본명					수정명
		 * aaa.jpg			=> makao_20230411173532.jpg 형태로 반환
		 */
		...
		// 1+ 2+ 3 조합해서 수정파일명을 변수에 담기
		String changeName = "wagle_" + currentTime + "_" + ranNum + ext;
