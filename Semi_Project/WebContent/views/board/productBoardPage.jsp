...
<% ArrayList<Board> pageBoardProductList = (ArrayList)request.getAttribute("pageBoardProductList"); %>
...
<body>
    ...
    <div id="b_wrap">
        <div id="b_header">
            <div id="b_category_title" style="font-size: 40px;">상품 게시판</div>
        </div>
        <div id="b_content">
            ...
            <div id="b_content_board">
                <table border="1" style="margin: auto; margin-top: 20px;">
                    ...
                    <% if(pageBoardProductList.isEmpty()){ %>
                        ...
                        <% } else { %>
                            <% for(Board b : pageBoardProductList) { %>
                            <tr>
            ...
            <div id="b_footer_page" align="center">
                ...
                <button onclick="location.href='<%=contextPath %>/productBoard.bo?aPage=<%= currentPage - 1 %>'" id="pageBtn_before">&lt;</button>
                
                <% for(int i = startPage; i <= endPage; i++) { %>
	                <% if(currentPage != i) { %>
	                	<button class="pageBtn" onclick="location.href='<%=contextPath%>/productBoard.bo?aPage=<%= i %>'" class="pageBtn"><%= i %></button>
	                <% } else { %>
	                	<button class="pageBtn" disaled><%= i %></button>
	                <% } %>
                <% } %>
                <% if(currentPage != maxPage) { %>
                <button onclick="location.href='<%=contextPath %>/productBoard.bo?aPage=<%= currentPage + 1 %>'" id="pageBtn_next">&gt;</button>
                <% } %>
            </div>
        </div>
    </div>
