...
<% ArrayList<Board> pageList = (ArrayList)request.getAttribute("pageList"); %>
...
<body>
    ...
    <div id="b_wrap">
        <div id="b_header">
            <div id="b_category_title" style="font-size: 40px;">전체 게시판</div>
        </div>
        <div id="b_content">
            ...
            <div id="b_footer_page" align="center">
                ...
                <button onclick="location.href='<%=contextPath %>/allBoard.bo?aPage=<%= currentPage - 1 %>'" id="pageBtn_before">&lt;</button>
                
                <% for(int i = startPage; i <= endPage; i++) { %>
	                <% if(currentPage != i) { %>
	                	<button class="pageBtn" onclick="location.href='<%=contextPath%>/allBoard.bo?aPage=<%= i %>'" class="pageBtn"><%= i %></button>
	                <% } else { %>
	                	<button class="pageBtn" disaled><%= i %></button>
	                <% } %>
                <% } %>
                <% if(currentPage != maxPage) { %>
                <button onclick="location.href='<%=contextPath %>/allBoard.bo?aPage=<%= currentPage + 1 %>'" id="pageBtn_next">&gt;</button>
                <% } %>
            </div>
        </div>
    </div>
